...
const CompanyDashboard = ({ data }) => {
    ...
    // 1. Aggregate Data by Month (since 'data' is now granular rows)
    const aggregatedData = useMemo(() => {
        ...
        // Calculate Averages and Final Objects
        return Object.values(grouped).map(item => ({
            ...item,
            FRT: item.FRT_Count ? (item.FRT_Sum / item.FRT_Count).toFixed(0) : 0,
            Conversion: item.Chats ? ((item.Leads / item.Chats) * 100).toFixed(1) : 0
        }));
    }, [data]);


    // 2. Metrics Calculation (Grand Totals)
    const metrics = useMemo(() => {
        ...
        return {
            ...
            conversion: { value: avgConversion, change: null }, // Calculated from Total Leads / Total Chats
            frt: { value: avgFRT, change: null }, // Average of all records
    ...
    if (!aggregatedData.length) return <div>No data available</div>;
                ...
                <div className="chart-widget">
                    <ChartWidget title="Impact: FRT vs Volume">
                        <ComposedChart data={aggregatedData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <Tooltip content={<CustomTooltip />} />
                            <Legend />
                            <Bar yAxisId="left" dataKey="Chats" name="Chats" fill="rgba(0, 243, 255, 0.3)" radius={[4, 4, 0, 0]} barSize={40} />
